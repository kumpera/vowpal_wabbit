...
void inline add_feature(example& ex, uint64_t idx, unsigned char ns, uint64_t mask, uint64_t multiplier, G_GNUC_UNUSED bool audit=false)
{
  ex.feature_space[(int)ns].push_back(1.0f, (idx * multiplier) & mask);
}

void add_all_features(example& ex, example& src, unsigned char tgt_ns, uint64_t mask, uint64_t multiplier, uint64_t offset, G_GNUC_UNUSED bool audit=false)
{
...
// arc-hybrid System.
size_t transition_hybrid(Search::search& sch, uint64_t a_id, uint32_t idx, uint32_t t_id, uint32_t n G_GNUC_UNUSED)
{
  task_data *data = sch.get_task_data<task_data>();
...
void get_gold_actions(Search::search &sch, uint32_t idx, uint64_t n G_GNUC_UNUSED, v_array<action>& gold_actions)
{
  task_data *data = sch.get_task_data<task_data>();
