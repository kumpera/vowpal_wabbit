...
template<class T>
double regularizer_direction_magnitude(vw& all G_GNUC_UNUSED, bfgs& b, double regularizer, T& weights)
{
  double ret = 0.;
...
template<class T>
float direction_magnitude(vw& all G_GNUC_UNUSED, T& weights)
{
  //compute direction magnitude
  double ret = 0.;
  for (typename T::iterator iter = weights.begin(); iter != weights.end(); ++iter)
...
template <class T>
void finalize_preconditioner(vw& all G_GNUC_UNUSED, bfgs& b, float regularization, T& weights)
{
...
template<class T>
void regularizer_to_weight(vw& all G_GNUC_UNUSED, bfgs& b, T& weights)
{
  if (b.regularizers != nullptr)
  {
    for (typename T::iterator w = weights.begin(); w != weights.end(); ++w)
...
template<class T>
double derivative_in_direction(vw& all G_GNUC_UNUSED, bfgs& b, float* mem, int &origin, T& weights)
{
  double ret = 0.;
  for (typename T::iterator w = weights.begin(); w != weights.end();  ++w)
  {
...
template<class T>
void update_weight(vw& all G_GNUC_UNUSED, float step_size, T& w)
{
