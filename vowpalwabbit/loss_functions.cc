...
public:
  ...
  float getUpdate(float prediction, float label,float update_scale, float pred_per_update G_GNUC_UNUSED)
  {
    return 2.f*(label - prediction) * update_scale;
  }
...
public:
  ...
  float getRevertingWeight(shared_data* sd G_GNUC_UNUSED, float prediction G_GNUC_UNUSED, float eta_t G_GNUC_UNUSED)
  {
  ...
  float second_derivative(shared_data*, float prediction, float label G_GNUC_UNUSED)
  {
    float exp_prediction = expf(prediction);
